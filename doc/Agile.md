...
```markdown
## 📌 Descrição
Resumo claro do que foi implementado ou corrigido nesta mudança.

## 🔗 Issue Relacionada
Link para a história de usuário ou bug report (ex: Closes #25).
...
- [ ] Novos testes unitários foram criados? (Cobertura mantida ou aumentada)
- [ ] O código segue o padrão de estilo do projeto (Linter/Prettier)?
- [ ] Variáveis de ambiente (.env) novas foram documentadas?
- [ ] A documentação (README) foi atualizada se necessário?

## 📝 Notas Adicionais
Instruções específicas para testar (ex: "Rodar npm install para pegar a nova lib html-pdf-node").